...
BEGIN
  WITH all_stats AS (
    -- Step 1: Calculate next_scheduled for EVERY client (Global Stats)
    SELECT 
      c.id as client_id,
      c.is_internal,
  ...
  global_counts AS (
    -- Step 2: Aggregate global totals excluding internal agency from the usage count
    SELECT
      jsonb_build_object(
  ...
  filtered_list AS (
    -- Step 3: Get the detailed list for the UI (Includes internal agency)
    SELECT 
