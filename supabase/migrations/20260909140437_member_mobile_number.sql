...
-- It also lets the member maintain it themselves from Profile Settings, the
-- same way they already maintain their name and avatar.
--
...
-- get_team_members returns the number, but only to owner/admin (or the person
-- themselves). Gated in the function rather than the UI: this is contact data,
-- and hiding it in a component would still ship it over the wire to everyone.
