...
--
-- Why this is a DB change even though the plan's §2.1 said "no database
-- changes": resolveNotificationRoute() treats notifications.link as
-- authoritative and only falls back to its entity map when link is null. Every
-- task notification row is stamped '/tasks' by this trigger, so widening the
-- client-side map alone would never take effect. The link has to be stamped
