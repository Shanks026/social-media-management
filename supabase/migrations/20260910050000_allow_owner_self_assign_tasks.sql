...
-- by someone else stays exactly as blocked as before — this only removes the
-- restriction on assigning a task to yourself.
--
-- Verified in a rolled-back transaction before applying:
--   owner assigns task to self          -> now allowed
--   admin assigns task to the owner     -> still blocked
--   member assigns task to self         -> unaffected (was already allowed)
