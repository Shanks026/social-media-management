...
--
-- This is the security-critical phase. `comments` today lets every workspace
-- member both read AND write every comment, because neither policy is
-- entity-aware. Tasks are narrower than posts and campaigns — creator /
-- assignee / past participant / admin, per the tasks_select widened in Phase 1
-- — so both policies have to learn about entity_type together. Fixing only
-- SELECT would leave a non-participant able to post into a thread they cannot
-- read.

-- 1. Allow the new entity type.
...
-- 2. SELECT: task comments follow task visibility. The `exists` subquery is
--    evaluated under the caller's own rights, so tasks' RLS filters it — a
--    task the caller can't select simply isn't found, and its comments fall
--    out with it. post/campaign behaviour is unchanged.
...
-- 3. INSERT: the same scoping on the write side.
...
-- 4. Fan-out. The deployed function branches on 'post' vs. an unconditional
--    else that ASSUMES 'campaign' — a task comment would have fallen into it
--    and produced '/campaigns/<task_id>' with campaign-shaped copy. Making the
--    branch three-way is a bug fix, not an enhancement.
...
begin
  ...
  if new.entity_type = 'post' then
    ...
    -- DEFINER, so these reads see the whole set rather than the commenter's
    -- own RLS-visible slice. emit_notifications drops nulls, de-dupes and
    -- strips the actor, so overlap with the thread participants and mentions
    -- already in v_recipients is harmless.
