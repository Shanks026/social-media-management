...
--
-- Reproduced against the live schema as a plain member of one workspace:
--   my own team                -> 3 rows (expected)
--   a DIFFERENT workspace      -> 1 row  (should be 0)
--   their emails               -> returned in full
--
-- This is long-standing, not new — but the Phase 2 rewrite
-- (20260909190000) reproduced the functions verbatim from their deployed
-- definitions and carried the missing guard forward with them.
--
-- Fix: require the requested workspace to be the caller's own. Expressed as a
-- WHERE clause rather than a RAISE so the functions stay LANGUAGE sql and an
-- out-of-scope request returns zero rows — the same "you simply don't see it"
-- shape as RLS everywhere else in this app, and no change to the client
-- contract (useTeamMembers/useRemovedMembers already handle an empty list).
--
