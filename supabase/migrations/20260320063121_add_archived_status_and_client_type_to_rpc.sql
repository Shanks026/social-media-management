...
-- 1. Add ARCHIVED to clients status check constraint
ALTER TABLE clients DROP CONSTRAINT IF EXISTS clients_status_check;
ALTER TABLE clients ADD CONSTRAINT clients_status_check
  CHECK (status IN ('ACTIVE', 'PAUSED', 'ARCHIVED'));

-- 2. Update get_clients_with_pipeline to support p_status filtering and return client_type
CREATE OR REPLACE FUNCTION public.get_clients_with_pipeline(
  p_user_id uuid,
      ...
      ROUND(
        CASE
          WHEN COALESCE((
            ...
            WHERE t.client_id = c.id AND t.type = 'INCOME' AND t.status = 'PAID'
          ), 0) > 0
          THEN (
            COALESCE((
              SELECT SUM(t.amount) FROM transactions t
            ...
            COALESCE((
              SELECT SUM(t.amount) FROM transactions t
              WHERE t.client_id = c.id AND t.type = 'EXPENSE' AND t.status = 'PAID'
            ), 0)
          ) / (
            SELECT SUM(t.amount) FROM transactions t
            WHERE t.client_id = c.id AND t.type = 'INCOME' AND t.status = 'PAID'
