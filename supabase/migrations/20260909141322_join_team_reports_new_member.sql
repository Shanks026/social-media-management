...
--
-- Links are multi-use and re-opening one is a normal thing to do, so join_team
-- already returns success for a person who had joined before. The welcome
-- email (send-team-welcome, invoked from JoinTeam.jsx) must not fire again for
-- them — the same duplicate-message problem already fixed for the "New team
-- member joined" notification, which is gated on the identical v_inserted check.
--
-- Body is otherwise the mobile-number version (20260910000000) unchanged; only
-- the returned jsonb gains a field.
