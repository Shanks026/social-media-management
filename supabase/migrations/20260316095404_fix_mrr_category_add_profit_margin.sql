...
BEGIN
  ...
  filtered_list AS (
    SELECT 
      ...
      -- Avg Monthly Retainer: sum of PAID INCOME transactions with retainer-like category ÷ months of relationship
      ROUND(
        COALESCE(
          (
            SELECT SUM(t.amount)
      ...
      -- Profit margin: (paid income - paid expenses) / paid income * 100
      ROUND(
        CASE
          WHEN COALESCE((
