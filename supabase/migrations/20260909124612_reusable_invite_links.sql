...
-- everyone behind them. The seat limit, not the token, is what caps how many
-- people can actually land: join_team already re-checks max_team_members per
-- join and that is unchanged.
--
...
-- Retire every currently-live link. Previously-accepted ones (including an
-- admin-role link) would otherwise silently come back to life the moment the
-- accepted_at filter is dropped, and every existing link was handed out under
-- single-use expectations, so none should quietly become a reusable admission
-- ticket. The owner issues fresh links after this. Uses the same mechanism as
-- useRevokeInvite: backdated expires_at.
update public.agency_invites
set expires_at = now()
where expires_at > now();

-- Seed the counter for links used under the old single-use rule.
update public.agency_invites
set use_count = 1
