...
serve(async (req) => {
  ...
  try {
    ...
    const branding = await getBranding(supabase, clientOwner?.user_id || record.created_by)
    ...
    if (record.status === 'SCHEDULED' || record.status === 'NEEDS_REVISION') {
      ...
      if (record.status === 'SCHEDULED' && clientInfo?.is_internal === true) {
        return new Response('Skipped: internal post self-scheduled, no notification needed.', { headers: corsHeaders })
      }

      const { data: creatorData } = await supabase.auth.admin.getUserById(record.created_by)
      const creatorEmail = creatorData?.user?.email
      if (!creatorEmail) {
        return new Response('Creator email not found', { status: 400, headers: corsHeaders })
      }
    ...
    if (record.status === 'PUBLISHED') {
      ...
      if (!clientInfo?.email) {
        return new Response('Client email not found', { status: 400, headers: corsHeaders })
      }
    ...
    return new Response('Ignored: Status change does not require notification.', { headers: corsHeaders })
  } catch (err: any) {
