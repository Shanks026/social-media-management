...
**Teams (`/team`):** Its own route (`TeamPage`), **not** a Settings tab — `Settings.jsx`'s `VALID_TABS` is `['profile','agency','invoice','danger']`, and an unrecognised `?tab=` value silently falls back to Profile. Agency owners invite teammates via a generated link (`/join/:token`); invite creation is **owner-only** (enforced in `agency_invites` RLS — admins cannot invite). Invited members get full workspace access scoped by role. The multi-tenant workspace model is the key architectural concept: all DB queries are scoped by `workspaceUserId` (the owner's UID), resolved via `get_my_agency_user_id()` SECURITY DEFINER SQL function. API module: `src/api/team.js` — which also owns the shared invite defaults (`DEFAULT_INVITE_EXPIRY_DAYS`, `MAX_INVITE_EXPIRY_DAYS`, `DEFAULT_INVITE_PERMISSIONS`) that `useGenerateInvite()` applies, so every caller issues identical invites. Public join page: `src/pages/JoinTeam.jsx`; team UI: `src/pages/TeamPage.jsx`, with the reusable `InviteDialog` / `EditAccessDialog` exported from `src/pages/settings/TeamSettings.jsx` (onboarding's team step reuses `InviteDialog` rather than implementing its own form). Both `useTeamMembers()` and `usePendingInvites()` maintain Supabase Realtime subscriptions for live updates.
